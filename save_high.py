...
low_obs_normalizer = obs_normalizer = LowLevelNormalizer(
    num_scan_stack=1,
    range_max=6.0,
...
low_expl_policy = LowLevelConvTanhGaussianPolicy(
    num_scan_stack=1,
    obs_normalizer=obs_normalizer
)
low_expl_policy.load_state_dict(low_params['exploration/policy'])
low_eval_policy = MakeDeterministic(low_expl_policy)
low_eval_policy.load_state_dict(low_params['evaluation/policy'])
...
qf1.load_state_dict(high_params['trainer/qf1'])
qf2 = HighLevelConvQf(
    num_scan_stack=1,
    obs_normalizer=obs_normalizer
)
qf2.load_state_dict(high_params['trainer/qf2'])
target_qf1 = HighLevelConvQf(
    num_scan_stack=1,
    obs_normalizer=obs_normalizer
)
target_qf1.load_state_dict(high_params['trainer/target_qf1'])
target_qf2 = HighLevelConvQf(
    num_scan_stack=1,
    obs_normalizer=obs_normalizer
)
target_qf2.load_state_dict(high_params['trainer/target_qf2'])
expl_policy = HighLevelConvTanhGaussianPolicy(
    num_scan_stack=1,
    obs_normalizer=obs_normalizer
)
expl_policy.load_state_dict(high_params['exploration/policy'])
eval_policy = MakeDeterministic(expl_policy)
eval_policy.load_state_dict(high_params['evaluation/policy'])
